...
/** The person. The app is Anime Buddy; he answers as Ren. */
export const BUDDY_NAME = "Ren";
export const BUDDY_NICK = "Buddy";
...
/** On-topic: anime, manga, watch, characters, figures. */
const LANE_HINT =
...
export function lockedReply(userText: string): string {
  if (looksPolish(userText)) {
    return "Niezły strzał. Jestem Ren. Co oglądamy?";
  }
...
export function mockBuddyReply(userText: string, context?: BuddyContext): string {
  ...
  const pl = looksPolish(userText);
  const picks = context?.catalogPicks ?? [];
  if (picks.length) {
    const names = picks.map((p) => p.title).join(", ");
    return pl
      ? `Z katalogu, nie z powietrza: ${names}. Karty są pod spodem — weź to, co pasuje, albo daj inny nastrój.`
      : `From the catalog, not from thin air: ${names}. Cards are underneath — grab one or give me a different mood.`;
  }
  return pl
    ? "Jestem Ren. Powiedz nastrój albo tytuł, który już lubisz — dobiorę z katalogu, nic nie zmyślam."
    : "I'm Ren. Give me a mood or a title you already love — I'll pick from the catalog. I don't invent shows.";
}
