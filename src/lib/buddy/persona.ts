...
/** High-precision identity-hijack detector. Normal anime talk must pass. */
...
export function mockBuddyReply(userText: string, context?: BuddyContext): string {
  if (isJailbreakAttempt(userText)) return lockedReply(userText);
  const pl = looksPolish(userText);
  const picks = context?.catalogPicks ?? [];
  if (picks.length) {
    const names = picks.map((p) => p.title).join(", ");
    return pl
      ? `Z katalogu bierze się to, nie z powietrza: ${names}. Karty są pod spodem — weź to, co pasuje, albo daj inny nastrój.`
      : `From the catalog, not from thin air: ${names}. Cards are underneath — grab one or give me a different mood.`;
  }
  return pl
    ? "Jestem Buddy. Powiedz nastrój albo tytuł, który już lubisz — dobiorę z katalogu, nic nie zmyślam."
    : "I'm Buddy. Give me a mood or a title you already love — I'll pick from the catalog. I don't invent shows.";
}
